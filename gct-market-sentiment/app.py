...
# Page config
st.set_page_config(page_title="GCT Market Sentiment", page_icon="📈", layout="wide")
...
def init_pipeline():
    ...
    if use_mock:
        st.warning(
            "No Tiingo API token found. Using mock data. Set TIINGO_API_TOKEN to use real data."
        )
...
with st.sidebar:
    ...
    time_range = st.selectbox(
        "Time Range", ["Last 24 Hours", "Last 7 Days", "Last 30 Days"], index=0
    )
...
# Display key metrics
with col1:
    st.metric("Total Articles Analyzed", stats.get("total_articles", 0), delta=None)

with col2:
    avg_coherence = stats.get("avg_coherence", 0) or 0
    st.metric("Average Coherence", f"{avg_coherence:.3f}", delta=None)
...
with col3:
    ...
    st.metric("Bullish Sentiment %", f"{bullish_pct:.1f}%", delta=None)
...
if all_tickers:
    selected_ticker = st.selectbox(
        "Select Ticker for Detailed Analysis", sorted(list(all_tickers))
    )
    ...
    for article in recent_articles:
        ...
        if article_with_scores and article_with_scores.get("coherence") is not None:
            articles_data.append(
                {
                    "Time": pd.to_datetime(article["timestamp"]).strftime(
                        "%Y-%m-%d %H:%M"
                    ),
                    "Title": (
                        article["title"][:80] + "..."
