...
class GCTVariableExtractor:
    ...
    def extract_psi(self, text: str) -> float:
        ...
        entities = [
            ent for ent in doc.ents if ent.label_ in ["ORG", "MONEY", "PERCENT", "DATE"]
        ]
    ...
    def extract_rho(self, text: str) -> float:
        ...
        conditional_count = sum(
            1 for token in doc if token.text.lower() in conditional_markers
        )
        ...
        rho = (causal_count + conditional_count * 1.5 + comparative_count * 2) / (
            text_length / 100
        )
    ...
    def extract_q_raw(self, text: str) -> float:
        ...
        strong_positive = sum(
            1 for term in self.financial_entities["bullish_terms"] if term in text_lower
        )
        strong_negative = sum(
            1 for term in self.financial_entities["bearish_terms"] if term in text_lower
        )
        uncertainty = sum(
            1
            for term in self.financial_entities["uncertainty_terms"]
    ...
    def extract_f(self, text: str) -> float:
        ...
        collective_count = sum(
            1 for token in doc if token.text.lower() in collective_pronouns
        )
        ...
        f = (collective_count + consensus_count * 1.5 + social_count * 2) / (
            text_length / 50
        )
