...
class TiingoClient:
    ...
    async def stream_realtime_news(
        self, callback: Callable[[Dict], None], tickers: List[str] = None
    ):
        """
        ...
        async with websockets.connect(self.ws_url) as websocket:
            ...
            subscribe_msg = {
                "eventName": "subscribe",
                "eventData": {"thresholdLevel": 5},  # Get all news
            }
    ...
    async def _fetch_price(
        self, session: aiohttp.ClientSession, ticker: str, url: str, params: Dict
    ) -> Optional[List]:
