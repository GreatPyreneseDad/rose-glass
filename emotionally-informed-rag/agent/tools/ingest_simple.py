#!/usr/bin/env python3
"""
Simplified document ingestion for Python 3.13 (without torch/sentence-transformers)

Uses simple TF-IDF-based embeddings as a fallback for full sentence-transformers.
"""
...
class SimpleDocumentIngester:
    """Ingest documents with simple TF-IDF embeddings (no torch required)"""
    ...
    def __init__(
        ...
        # Build vocabulary from common legal terms
    ...
    def _build_legal_vocabulary(self) -> List[str]:
        ...
        return [
            ...
            # Time/process terms
            "urgent", "immediate", "emergency", "deadline", "timeline", "schedule",
            "pending", "ongoing", "current", "past", "future", "recent",
            # Relational terms
            "family", "parent", "child", "partner", "spouse", "relationship",
            "communication", "conflict", "resolution", "mediation", "negotiation"
        ]
    ...
    def _simple_embedding(self, text: str) -> np.ndarray:
        ...
        This is a basic fallback - not as good as sentence-transformers,
        but works without torch and provides reasonable semantic similarity
        """
        ...
        for idx, vocab_term in enumerate(self.vocabulary[:self.embedding_dim]):
            if vocab_term in word_counts:
                ...
                idf = 1.0  # Simplified - would need document frequency for true IDF
                embedding[idx] = tf * idf

        # Add some randomness based on non-vocabulary words for better differentiation
        non_vocab_words = [w for w in words if w not in self.vocabulary]
        if non_vocab_words:
            # Use hash-based embedding for non-vocabulary terms
            for word in non_vocab_words[:50]:  # Limit to avoid too much noise
                # Map word to indices using hash
                hash_val = hash(word)
    ...
    def _analyze_emotional_signature(self, text: str) -> Dict:
        ...
        if self.rose_glass:
            try:
                ...
                logger.warning(f"Rose Glass analysis failed: {e}, using fallback")
                # Fall through to basic heuristics
    ...
    def ingest_directory(
        ...
        Args:
            ...
            exclude_patterns: List of patterns to exclude (e.g., ['node_modules', 'venv'])
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Ingest documents with simple embeddings (Python 3.13 compatible)'
    )
    ...
    parser.add_argument('--exclude', nargs='+', default=[],
                       help='Patterns to exclude (e.g., node_modules venv __pycache__)')
